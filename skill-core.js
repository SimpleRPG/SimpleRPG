...
// ★ バグ修正: shieldBlowGuardTurnRemain は game-core-3.js の winBattle / enemyTurn で
//   参照されるため、ここで宣言する（game-core-3 より後に読み込まれるなら問題なし）
//   game-core-3 側の enemyTurn でこの変数を参照するので、グローバルとして宣言
...
function tickSkillBuffTurns() {
  ...
  if (shieldBlowGuardTurnRemain > 0) {
    // シールドブロウは「次の被ダメ1回」なので、実際の軽減処理側（enemyTurn）で0にする
  }
...
function doPetTurn() {
  ...
  // ★ バグ修正: winBattle() は game-core-3.js で定義済みなので正しく呼び出せる
  if (enemyHp <= 0) {
    winBattle();
  } else {
    updateDisplay();
...
function castMagicFromUI() {
  ...
  if (skillId !== "beastHeal") {
    if (enemyHp <= 0) {
      // ★ バグ修正: winBattle() は game-core-3.js で定義済み
      winBattle();
    } else {
...
// =======================
// 職業ごとのスキルUI表示切り替え
// ★ バグ修正: updateBattleSkillUIByJob と updateSkillButtonsByJob が重複・矛盾していた
//   → updateBattleSkillUIByJob を正として統一し、updateSkillButtonsByJob は
//      updateBattleSkillUIByJob を呼ぶラッパーに変更
// =======================
...
// ★ バグ修正: 重複・矛盾していた updateSkillButtonsByJob を
//   updateBattleSkillUIByJob への委譲に統一
